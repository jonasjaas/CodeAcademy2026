...
await channel.QueueDeclareAsync(queue: "topic_queue_3", durable: true, exclusive: false, autoDelete: false, arguments: null);
await channel.QueueBindAsync(queue: "topic_queue_3", exchange: "topic_exchange", routingKey: "idem.public.reply", arguments: null);
...
// Start consuming messages
await channel.BasicConsumeAsync(queue: "topic_queue_3", autoAck: false, consumerTag: "", noLocal: false, exclusive: false, arguments: null, consumer: consumer);
Console.ReadLine(); // Keep the application running to listen for messages